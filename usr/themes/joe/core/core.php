...
function ParseCode($text)
{
    ...
    $text = preg_replace_callback('/<img.*?src=\"(.*?)\".*?>/i', function ($text) {
        return '<a no-target data-fancybox="gallery" href="' . $text[1] . '"><img class="lazyload" src="' . GetLazyLoad()  . '" data-original="' . $text[1] . '"></a>';
    }, $text);
...
function Short_Card_Nav($text)
{
    ...
    $text = preg_replace_callback('/\[card-nav-item.*?src="(.*?)".*?title="(.*?)".*?img="(.*?)".*?\/\]/ism', function ($text) {
        ...
        $arr = array(
            0 => "linear-gradient(33deg, #6DE195, #C4E759)",
            1 => "linear-gradient(33deg, #41C7AF, #54E38E)",
            2 => "linear-gradient(33deg, #99E5A2, #D4FC78)",
            3 => "linear-gradient(33deg, #ABC7FF, #C1E3FF)",
            4 => "linear-gradient(33deg, #6CACFF, #8DEBFF)",
            5 => "linear-gradient(33deg, #5583EE, #41D8DD)",
            6 => "linear-gradient(33deg, #A16BFE, #DEB0DF)",
            6 => "linear-gradient(33deg, #D279EE, #F8C390)",
            7 => "linear-gradient(33deg, #F78FAD, #FDEB82)",
            8 => "linear-gradient(33deg, #BC3D2F, #A16BFE)",
            9 => "linear-gradient(33deg, #A43AB2, #E13680)",
            10 => "linear-gradient(33deg, #9D2E7D, #E16E93)"
        );
        return '<div class="item">
                    <a href="' . $text[1] . '" class="nav" style="background-image: ' . $arr[rand(0, 10)] . '">
